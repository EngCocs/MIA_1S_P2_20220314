...
func GenerarReporteBlocks(particion Partition, disco *os.File) string {
	var sb Superblock
	err := Herramientas.ReadObject(disco, &sb, int64(particion.Start))
	if err != nil {
	...
	for i := int32(0); i < sb.S_blocks_count; i++ {
		posBitmap := int64(sb.S_bm_block_start) + int64(i)
		var bit [1]byte
		if _, err := disco.ReadAt(bit[:], posBitmap); err != nil {
			continue
		}
		if bit[0] != 1 {
			continue
		...
		label := ""
		blockPos := int64(sb.S_block_start) + int64(i)*int64(sb.S_block_size)

		switch {
		case i < sb.S_blocks_count:
			// Intentamos leer como Folderblock
			var folder Folderblock
			if err := Herramientas.ReadObject(disco, &folder, blockPos); err == nil {
				label += fmt.Sprintf("Bloque Carpeta %d|", i)
				for j := 0; j < 4; j++ {
					name := GetB_name(string(folder.B_content[j].B_name[:]))
					inodo := folder.B_content[j].B_inodo
					if inodo != -1 {
						label += fmt.Sprintf("<f%d> %s | Inodo: %d\\l", j, name, inodo)
					}
				}
			} else {
				// Intentamos leer como Fileblock
				var file Fileblock
				if err := Herramientas.ReadObject(disco, &file, blockPos); err == nil {
					contenido := GetB_content(string(file.B_content[:]))
					label += fmt.Sprintf("Bloque Archivo %d|%s\\l", i, contenido)
				} else {
					// Intentamos leer como Pointerblock
					var apuntador Pointerblock
					if err := Herramientas.ReadObject(disco, &apuntador, blockPos); err == nil {
						label += fmt.Sprintf("Bloque Apuntador %d|", i)
						for j := 0; j < 16; j++ {
							if apuntador.B_pointers[j] != -1 {
								label += fmt.Sprintf("<p%d> Apunta a: %d\\l", j, apuntador.B_pointers[j])
							}
						}
					}
				}
		...
		if label != "" {
			dot += fmt.Sprintf("block%d [label=\"{%s}\"];\n", contador, label)
			contador++
		}
	}
...
func GenerarReporteInodes(particion Partition, disco *os.File) string {
	var sb Superblock
	err := Herramientas.ReadObject(disco, &sb, int64(particion.Start))
	if err != nil {
		fmt.Println("Error: No se pudo leer el superbloque")
	...
	for i := int32(0); i < sb.S_inodes_count; i++ {
		posBM := int64(sb.S_bm_inode_start) + int64(i)
		var bit [1]byte
		if _, err := disco.ReadAt(bit[:], posBM); err != nil || bit[0] != 1 {
			continue
		}

		inodoPos := int64(sb.S_inode_start) + int64(i)*int64(sb.S_inode_size)
		var inodo Inode
		if err := Herramientas.ReadObject(disco, &inodo, inodoPos); err != nil {
			continue
		}

		dot += fmt.Sprintf("inode%d [label=<<table border='1' cellborder='1' cellspacing='0'>\n", i)
		dot += "<tr><td colspan='2'><b>Inodo " + fmt.Sprint(i) + "</b></td></tr>\n"
		dot += fmt.Sprintf("<tr><td>UID</td><td>%d</td></tr>\n", inodo.I_uid)
		dot += fmt.Sprintf("<tr><td>GID</td><td>%d</td></tr>\n", inodo.I_gid)
		dot += fmt.Sprintf("<tr><td>Size</td><td>%d</td></tr>\n", inodo.I_size)
		dot += fmt.Sprintf("<tr><td>Atime</td><td>%s</td></tr>\n", string(inodo.I_atime[:]))
		dot += fmt.Sprintf("<tr><td>Ctime</td><td>%s</td></tr>\n", string(inodo.I_ctime[:]))
		dot += fmt.Sprintf("<tr><td>Mtime</td><td>%s</td></tr>\n", string(inodo.I_mtime[:]))
		dot += fmt.Sprintf("<tr><td>Type</td><td>%s</td></tr>\n", string(inodo.I_type[:]))
		dot += fmt.Sprintf("<tr><td>Perm</td><td>%s</td></tr>\n", string(inodo.I_perm[:]))
		for j := 0; j < 15; j++ {
			dot += fmt.Sprintf("<tr><td>Block[%d]</td><td>%d</td></tr>\n", j, inodo.I_block[j])
		}
		dot += "</table>>];\n"
	}
	dot += "}"
